...
const Banner = () => {
  ...
  const slides = [
    {
      ...
      icon: <FaShoppingCart className="text-6xl text-[#00f0ff] mb-6" />, // use an e-commerce icon
      particles: 22,
    },
    {
  ...
  return Array.from({ length: count }).map((_, i) => {
    const size = Math.random() * 6 + 2; // numeric
    return (
      <motion.div
        key={i}
        className="absolute rounded-full bg-[#00f0ff]"
        style={{
          width: size,   // number, NOT "px"
          height: size,  // number, NOT "px"
  ...
  return (
    <div
      ref={bannerRef}
      className="relative w-full h-[60vh] sm:h-[70vh] md:h-[60vh] lg:h-[65vh] xl:h-[80vh] overflow-hidden"
    >
